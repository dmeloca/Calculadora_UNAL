...
def cancelacion(bolsa_creditos, carrera):
    num_canceladas = int(input("[?] Cuántas materias canceló: "))
    for a in range(num_canceladas):
        creds = int(input(f"[?] Ingrese el número de créditos que canceló en la materia {a+1}: "))
        bolsa_creditos -= creds
    return bolsa_creditos
...
def creditosfnl(usuario, carrera):
    ...
    for materia in usuario:
        ...
        bolsadecreditos1 = cancelacion(bolsadecreditos1, carrera)*2
...
def porcentajeAvance(materiasAprobadas, carrera):
    ...
    if creditos_aprobados > 0:
        ...
        print( f"[!] Su porcentaje de avance es de: {(creditos_aprobados/creditostotales)*100}%")
        cancelo = input("[?] Usted canceló materias (S/N)")
        if cancelo.lower() == "s":
            creditos_aprobados = cancelacion(creditos_aprobados, carrera)
            print(f'[$] Usted tiene {creditos_aprobados} creditos, luego de las cancelaciones')
        else:
            print(f'[$] Usted tiene {creditos_aprobados} creditos adicionales')
